...
public class BaseRestApiClient(HttpClient client, JsonSerializerOptions options)
{
    ...
    {
        using var jsonContent = GetStringContent(request);
    ...
    protected async Task DeleteAsync(string uri, CancellationToken token = default) =>
        await client.DeleteAsync(uri, token);
